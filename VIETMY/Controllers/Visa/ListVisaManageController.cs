...
namespace VIETMY.Controllers.Visa
{
    public class ListVisaManageController : BaseController
    {
        ...
        public ActionResult ExcelSample()
        {
            try
            {
                ...
                using (var stream = new MemoryStream())
                {
                    string templateFile = System.Web.HttpContext.Current.Server.MapPath("~\\Upload\\ImportExport\\Import_VisaVIETMY.xlsx");
                    ExportExcelTemplateVisa(stream, templateFile, header);
                    bytes = stream.ToArray();
                }

                string fileName = "Mau-import-visa-VIETMY.xlsx";
